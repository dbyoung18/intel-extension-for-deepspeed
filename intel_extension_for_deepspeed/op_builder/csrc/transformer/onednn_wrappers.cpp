                         ...
                         int batch)
{
  ...
  dnnl::memory::data_type dnnl_dtype;
  if (std::is_same<T, float>::value) {
    dnnl_dtype = dnnl::memory::data_type::f32;
  } else if (std::is_same<T, fp16>::value) {
    dnnl_dtype = dnnl::memory::data_type::f16;
  } else if (std::is_same<T, bf16>::value) {
    dnnl_dtype = dnnl::memory::data_type::bf16;
  } else {
    throw std::runtime_error("Unsupport dtype");
  }
  ...
  if constexpr (bmm) {
    src_md = dnnl::memory::desc(src_dims, dnnl_dtype,
                                trans_src ? dnnl::memory::format_tag::acb
                                          : dnnl::memory::format_tag::abc);
    wgt_md = dnnl::memory::desc(wgt_dims, dnnl_dtype,
                                trans_wgt ? dnnl::memory::format_tag::acb
                                          : dnnl::memory::format_tag::abc);
    dst_md = dnnl::memory::desc(dst_dims, dnnl_dtype,
                                dnnl::memory::format_tag::abc);
  } else {
    src_md = dnnl::memory::desc(src_dims, dnnl_dtype,
