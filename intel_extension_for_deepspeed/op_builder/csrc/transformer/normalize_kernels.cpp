...
#pragma unroll
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        vals_arr[i] = vals_arr[i] * gamma[i * iteration_stride + id] + beta[i * iteration_stride + id];
...
#pragma unroll
    for (int i = 0; i < iterations; i++) {
        vals_f[i] = residual_cast[i * iteration_stride + id].convert<float>();
        sum += vals_f[i].x();
        sum += vals_f[i].y();
    }
    if ((high_index) < row_stride) {
        vals_f[iterations] = residual_cast[high_index].convert<float>();
    ...
    half2 variance_h =
        vec<float, 2>({variance, variance}).convert<half>();
    ...
    if (training && sg.get_local_id() == 0) {
        vars[row] = vec<float, 1>(variance).convert<half>();
        if constexpr (is_mean) {
            means[row] = vec<float, 1>(mean).convert<half>();
        }
    }
    iterations = row_stride / iteration_stride;
    for (int i = 0; i < iterations; i++) {
        half2 vals_arr = vals_f[i].convert<half>();
    ...
    if ((high_index) < row_stride) {
        half2 vals_arr = vals_f[iterations].convert<half>();
        vals_arr = vals_arr * rsqrt(variance_h);
